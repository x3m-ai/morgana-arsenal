...
def setup_logger(level=logging.DEBUG):
    ...
    if no_color():
        logging.basicConfig(level=level, format=format, datefmt=datefmt, handlers=[file_handler])
    else:
        ...
        logging.basicConfig(
            ...
            format=format,
            datefmt=datefmt,
            handlers=[RichHandler(rich_tracebacks=True, markup=True, console=console), file_handler]
        )
    ...
    logging.getLogger("contact_http").setLevel(logging.DEBUG)
    logging.getLogger("contact_svc").setLevel(logging.DEBUG)
...
def run_tasks(services, run_vue_server=False):
    ...
    loop.create_task(app_svc.start_auto_save())

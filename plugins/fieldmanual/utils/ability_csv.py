...
def generate_ability_csv(caldera_dir, dest_file='abilities.csv'):
    ...
    with dest_path.open('w', newline='') as fle:
        ...
        for ability_file in caldera_path.glob('**/abilities/*/*.yml'):
            ...
            try:
                abilities = list(yaml.safe_load_all(ability_file.read_text()))
                if not abilities:
                    continue
                
                # Get first document, handle if it's already a list
                ability = abilities[0]
                if isinstance(ability, list) and len(ability) > 0:
                    ability = ability[0]
                    
            except Exception as e:
                print(f"Warning: Could not parse {ability_file}: {e}")
                continue
